...
from receiver_new_with_latency import Receiver
...
class WeedDetectionGUI:
    def __init__(self, root):
        ...
        #latency added
        self.lbl_latency = tk.Label(self.stats_frame, text="Latency: 0", font=("Arial", 12), fg="gray", bg="#f0f0f0")
        self.lbl_latency.pack(pady=5)
    ...
    def video_loop(self):
        while self.is_running:
            # Getting footage
            frame, count , latency_ms = self.backend.get_frame()
            ...
            if frame is not None:
                ...
                self.root.after(0, self.update_interface, imgtk, count, latency_ms)
            else:
                # If stream fails temporarily
                time.sleep(0.1)

    def update_interface(self, imgtk, count, latency):
        ...
        self.lbl_latency.config(text=f"Latency: {latency:.1f}")
